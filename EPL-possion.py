...
import Rfunc
# from margin_protect import Ui_MainWindow
...
for i in range(2, 13):
    if sh.range(i, 4).value is not None:
        score = matrix(sh.range(i, 14).value, sh.range(i, 15).value)
        HAD = had(score)
        HILO = hilo(score)
        home = sh.range(i, 4).value
        away = sh.range(i, 8).value
        j = i + 12
        ...
        r2 = [round(x, 3) for x in result]
        sh.range((j, 1), (j, 2)).value = [home, away]
        sh.range((j, 3), (j, 12)).value = r2
        r3 = margin_ui(result)
        sh.range((i, 16), (i, 18)).value = r3[:3]
        sh.range((i, 20), (i, 21)).value = r3[3:]
